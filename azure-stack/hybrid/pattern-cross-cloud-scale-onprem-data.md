...
| | Azure-Funktionen | Mit [Azure Functions](/azure/azure-functions/) können Sie Code in einer serverlosen Umgebung ausführen, ohne vorher eine VM erstellen oder eine Webanwendung veröffentlichen zu müssen. |
| | Automatische Skalierung in Azure | [Automatische Skalierung](/azure/azure-monitor/platform/autoscale-overview) ist ein integriertes Feature von Azure Cloud Services, Azure Virtual Machines und Web-Apps. Es ermöglicht Anwendungen, bei sich ändernden Anforderungen die bestmögliche Leistung zu bringen. Anwendungen passen sich an Datenverkehrsspitzen an, benachrichtigen Sie, wenn sich Metriken ändern, und werden nach Bedarf skaliert. |
| Azure Stack Hub | IaaS/Compute | Azure Stack Hub ermöglicht Ihnen, dasselbe Anwendungsmodell, Self-Service-Portal und die APIs zu verwenden, die von Azure aktiviert wurden. Azure Stack Hub IaaS bietet eine breite Palette von Open-Source-Technologien für einheitliche Hybrid Cloud-Bereitstellungen. Das Lösungsbeispiel verwendet beispielsweise eine Windows Server-VM für SQL Server.|
...
| Azure DevOps Services | Registrierung | Nutzen Sie das schnelle Einrichten von Continuous Integration für Build, Test und Bereitstellung. Weitere Informationen finden Sie unter [Registrieren und Anmelden bei Azure DevOps](/azure/devops/user-guide/sign-up-invite-teammates?view=azure-devops). |
...
- Erstellen Sie mit wenigen Klicks mehrschichtige Entwicklungsumgebungen in Azure, und replizieren Sie sogar aktuelle Produktionsdaten in Ihre Entwicklungs-/Testumgebung, um sie nahezu in Echtzeit synchron zu halten.

**Wirtschaftlichkeit der cloudübergreifenden Skalierung mit Azure Stack Hub**
