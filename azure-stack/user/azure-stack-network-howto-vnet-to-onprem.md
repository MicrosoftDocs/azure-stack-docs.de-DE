---
title: Einrichten eines VPN-Gateways für Azure Stack | Microsoft-Dokumentation
description: Es wird beschrieben, wie Sie ein VPN-Gateway für Azure Stack einrichten.
...
-  Zugriff auf ein VPN-Gerät im lokalen Netzwerk, auf dem das integrierte Azure Stack-System gehostet wird. Das Gerät muss einen IPSec-Tunnel erstellen, für den die unter [Bereitstellungsparameter](#deployment-parameters) beschriebenen Parameter erfüllt werden.

-  Eine NVA-Lösung (Network Virtual Appliance) in Ihrem Azure Stack-Marketplace. Mit einem virtuellen Netzwerkgerät (Network Virtual Appliance, NVA) wird der Fluss des Netzwerkdatenverkehrs aus einem Umkreisnetzwerk in andere Netzwerke oder Subnetze gesteuert. In diesem Verfahren wird die Lösung [FortiGate Next-Generation Firewall – Single VM](https://azuremarketplace.microsoft.com/marketplace/apps/fortinet.fortinet-FortiGate-singlevm) genutzt.
...
> [!Note]
> \* Wählen Sie einen anderen Adressraum und andere Subnetzpräfixe aus, wenn es für `172.16.0.0/16` zu einer Überschneidung mit dem lokalen Netzwerk oder dem Azure Stack-VIP-Pool kommt.
...
1. Erstellen Sie in den Azure Stack-VNETs einen virtuellen Computer und im lokalen Netzwerk ein System. Sie können die Anleitung zum Erstellen eines virtuellen Computers unter [Schnellstart: Erstellen eines virtuellen Windows Server-Computers mit dem Azure Stack-Portal](https://docs.microsoft.com/azure-stack/user/azure-stack-quick-windows-portal) befolgen.

2. Überprüfen Sie beim Erstellen der Azure Stack-VM und beim Vorbereiten des lokalen Systems Folgendes:

-  Die Azure Stack-VM wird im **InsideSubnet** des VNET angeordnet.

-  Das lokale System wird im lokalen Netzwerk innerhalb des definierten IP-Adressbereichs angeordnet, wie dies in der IPSec-Konfiguration definiert ist. Stellen Sie außerdem sicher, dass die IP-Adresse der lokalen Schnittstelle des lokalen VPN-Geräts für das lokale System als Route bereitgestellt wird, über die das Azure Stack-VNET erreichbar ist, z. B. `172.16.0.0/16`.

-  Wenden Sie **keine** NSGs auf den zu erstellenden virtuellen Azure Stack-Computer an. Unter Umständen müssen Sie die NSG entfernen, die standardmäßig hinzugefügt wird, wenn die VM über das Portal erstellt wird.

-  Stellen Sie sicher, dass die Betriebssysteme des lokalen Systems und der Azure Stack-VM keine Firewallregeln für Betriebssysteme aufweisen, durch die die Kommunikation verhindert wird, die Sie zum Testen der Konnektivität verwenden. Wir empfehlen Ihnen, zu Testzwecken die Firewall im Betriebssystem beider Systeme vollständig zu deaktivieren.
