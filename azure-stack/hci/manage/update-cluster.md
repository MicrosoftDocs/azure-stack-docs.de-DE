...
Beim Aktualisieren von Azure Stack HCI-Clustern besteht das Ziel darin, die Verfügbarkeit aufrechtzuerhalten, indem jeweils nur ein Server im Cluster aktualisiert wird. Bei vielen Betriebssystemupdates ist es erforderlich, den Server offline zu schalten, um z. B. einen Neustart auszuführen oder Software wie den Netzwerkstapel zu aktualisieren. Es wird empfohlen, das Feature [Clusterfähiges Aktualisieren](/windows-server/failover-clustering/cluster-aware-updating) zu nutzen, das den Softwareaktualisierungsprozess automatisiert, sodass Sie Windows-Updates leichter auf jedem einzelnen Server in Ihrem Cluster installieren können, ohne die Ausführung Ihrer Anwendungen zu unterbrechen. Das clusterfähige Aktualisieren kann in allen Editionen von Windows Server – einschließlich Server Core-Installationen – verwendet und über Windows Admin Center oder PowerShell initiiert werden.
